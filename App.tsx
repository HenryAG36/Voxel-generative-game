...
import { AppState, WorldManifesto, GameEntity } from './types';
import { GoogleGenAI, Type } from "@google/genai";
import { Shield, Zap, Sword, Move, Brain, Loader2, Info, AlertTriangle } from 'lucide-react';
...
const LoadingScreen: React.FC<{ message: string; progress: number }> = ({ message, progress }) => {
  ...
  return (
    <div className="absolute inset-0 z-[200] bg-slate-950 flex flex-col items-center justify-center p-12 overflow-hidden">
      ...
      {/* Decorative Code Streams */}
      <div className="absolute bottom-12 left-12 font-mono text-[8px] text-indigo-900/40 space-y-1">
        <div>GEN_SEED_0x8F92</div>
        <div>MAPPING_VOXEL_DATA_ARRAY</div>
        <div>CALC_COLLISION_BOUNDS</div>
        <div>SUMMON_AI_LOGIC_CORE</div>
      </div>
    </div>
  );
};
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (!engineRef.current) engineRef.current = new VoxelEngine(containerRef.current);
    ...
    const updateMovement = () => {
        ...
        engineRef.current.setInputDirection(dir.length() > 0 ? dir.normalize() : new THREE.Vector3(0, 0, 0));
    };
  ...
  const executeSkill = (index: number) => {
      ...
      if (target) engineRef.current.damageEntity(target.id, skill.damage);
  };
  ...
  const initWorld = async (data: { name: string; ambient: string; lore: string }) => {
    ...
    try {
        ...
        setWorld({ ...data, palette: result.palette });
        ...
        await generateEntities({ ...data, palette: result.palette, chunks: result.chunks });
    } catch (e) {
        console.error(e);
  ...
  const generateEntities = async (manifesto: any) => {
  ...
  const createHero = async (prompt: string) => {
    ...
    try {
        ...
        const hero: GameEntity = {
            ...
            stats: { ...result.stats, maxHp: result.stats.hp, speed: 12, defense: 10, description: result.stats.description || '' },
            isDead: false
        };
        setPlayer(hero);
  ...
  return (
    <div className="relative w-full h-screen bg-slate-950 overflow-hidden" onMouseMove={handleJoystickMove} onMouseUp={handleJoystickEnd} onTouchMove={handleJoystickMove} onTouchEnd={handleJoystickEnd}>
      ...
      {gameState === AppState.WIZARD && <Wizard onComplete={initWorld} />}
      ...
      {gameState === AppState.PLAYING && player && (
          <>
            <HUD player={player} worldName={world?.name || "The Void"} onSkillUse={executeSkill} />
            ...
            {player.isDead && (
                <div className="absolute inset-0 z-[300] bg-red-950/80 backdrop-blur-md flex flex-col items-center justify-center p-12 text-center">
                    ...
                    <button onClick={()=>window.location.reload()} className="px-12 py-5 bg-white text-red-950 font-black rounded-3xl shadow-2xl hover:bg-red-50 transition-all active:scale-95">REAWAKEN CORE</button>
                </div>
            )}
          </>
      )}
...
const HUD = ({ player, worldName, onSkillUse }: any) => {
            ...
            <div className="flex justify-between items-start">
                <div className="bg-slate-900/90 backdrop-blur-xl border border-slate-700 p-6 rounded-[2.5rem] min-w-[320px] pointer-events-auto shadow-2xl">
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <div className="flex items-center gap-1.5 text-[10px] text-slate-500 font-bold uppercase"><Info size={10} /> Live Simulation</div>
                    </div>
                    ...
                    <div className="flex gap-4 mt-6">
                        <Stat icon={<Sword size={12}/>} label="POW" value={player.stats.power} />
                        <Stat icon={<Shield size={12}/>} label="DEF" value={player.stats.defense} />
                        <Stat icon={<Zap size={12}/>} label="SPD" value={player.stats.speed} />
                    </div>
                </div>
            </div>
...
const Stat = ({icon, label, value}: any) => (
    <div className="flex flex-col flex-1 items-center justify-center bg-slate-800/40 p-2.5 rounded-2xl border border-slate-700/50">
